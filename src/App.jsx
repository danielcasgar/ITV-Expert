...
function App() {
  ...
  const [protocoloHibridoPDF, setProtocoloHibridoPDF] = useState(null);
  ...
  // Lee cualquier archivo (por ejemplo, el PDF del Protocolo Híbrido) y lo convierte a base64 tal cual, sin comprimir.
...
const hacerLogin = () => {
  ...
  const handleProtocoloHibrido = async (e, isEdit = false) => {
    const file = e.target.files[0];
    if (!file) return;
    ...
    if (file.size > 500000) {
      alert("El archivo PDF pesa demasiado (máximo recomendado: 500 KB) para guardarlo en la base de datos. Intenta usar un PDF más ligero.");
      return;
    }
    try {
      const base64 = await leerArchivoBase64(file);
      if (isEdit) {
        setVehiculoDetalle(prev => ({ ...prev, protocoloHibridoPDF: base64 }));
      } else {
        setProtocoloHibridoPDF(base64);
      }
    } catch (error) {
  ...
  const guardarVehiculo = async () => {
    if (!esMaestro) return alert("Solo la cuenta Maestra puede añadir vehículos.");
    ...
    const vehiculoFinal = { 
      ...
      protocoloHibridoPDF: protocoloHibridoPDF || null,
      bloqueado: false,
      modificadoPor: { 
    ...
    try {
      ...
      const fileProtocolo = document.getElementById('protocolo-hibrido-nuevo'); if (fileProtocolo) fileProtocolo.value = '';
...
const parsearDiametros = (medidaTexto) => {
  ...
  const generarInformeNeumaticos = () => {
    ...
    ejesActivos.forEach(eje => {
      ...
      claves.forEach(clave => {
        const valor = (eje.ruedas[clave] || '').trim();
        if (valor) todasLasRuedas.push({ ejeId: eje.id, clave, valor });
      });
    });

    if (todasLasRuedas.length === 0) {
      alert("No hay ninguna medida introducida para copiar.");
      return;
    }
    ...
    if (todasLasRuedas.length === 1) {
      ...
      ejesActivos.forEach(eje => {
        const claves = eje.gemela ? ['izqExt', 'derExt', 'izqInt', 'derInt'] : ['izq', 'der'];
        const rellenas = claves.map(clave => ({ clave, valor: (eje.ruedas[clave] || '').trim() })).filter(r => r.valor);
    ...
    const limpiarCasillas = () => {
      setEjesNeumaticos(estadoEjesInicial);
    ...
    if (navigator.clipboard && navigator.clipboard.writeText) {
      navigator.clipboard.writeText(texto).then(() => {
        ...
        alert("No se ha podido copiar automáticamente. Informe:\n\n" + texto);
        limpiarCasillas();
      });
    } else {
      alert("No se ha podido copiar automáticamente. Informe:\n\n" + texto);
  ...
  const renderBotonesModo = (cat, pdfProtocolo, onProtocoloSeleccionado, protocoloInputId) => {
    ...
    return (
      <div className="flex flex-wrap gap-2 justify-center">
        ...
        {onProtocoloSeleccionado && (
          <>
            <input type="file" id={protocoloInputId} className="hidden" accept="application/pdf" onChange={onProtocoloSeleccionado} />
            <label htmlFor={protocoloInputId} className={`flex-1 min-w-[70px] py-4 rounded-xl text-[8px] font-black uppercase transition-all text-center cursor-pointer bg-gray-300 text-black ${pdfProtocolo ? 'ring-2 ring-green-500' : ''}`}>
              Protocolo Híbrido{pdfProtocolo ? ' ✓' : ''}
            </label>
          </>
        )}
      </div>
  ...
  const renderRuedaInput = (eje, clave, label) => {
    ...
    return (
      <div className="flex-1 min-w-[120px]">
        ...
        <input 
          type="text" 
          placeholder="Ej: 205/55R16, 205/55ZR16 o 205/55-16" 
...
<div className="space-y-6">
          ...
          <>
            ...
            {editandoVehiculo ? (
              <div className="space-y-6 animate-fadeIn">
                ...
                {renderBotonesModo(vehiculoDetalle.categoria || 'M/N', vehiculoDetalle.protocoloHibridoPDF, (e) => handleProtocoloHibrido(e, true), 'protocolo-hibrido-edit')}
    ...
    if (paginaActual === 'añadir' && esMaestro) return (
      <div className="space-y-6">
        <div className="bg-[#101c33] p-6 rounded-3xl border border-gray-800 shadow-xl">
        ...
        <div className="bg-[#060c17] p-4 rounded-3xl border border-gray-800">
          ...
          {renderBotonesModo(nuevoVehiculo.categoria, protocoloHibridoPDF, (e) => handleProtocoloHibrido(e, false), 'protocolo-hibrido-nuevo')}
        </div>
    ...
    if (paginaActual === 'contacto') return (
      <div className="space-y-6 animate-fadeIn">
        <div className="bg-[#101c33] p-8 rounded-[2.5rem] border border-gray-800 shadow-2xl text-center relative overflow-hidden">
          <div className="absolute -top-10 -right-10 w-32 h-32 rounded-full bg-[#2980b9] opacity-[0.05]"></div>
          <h2 className="text-2xl font-black italic uppercase text-[#2980b9] mb-6 relative z-10">Contacto</h2>
          <p className="text-sm text-gray-300 font-bold leading-relaxed mb-8 relative z-10">
          ...
          <a href="mailto:danielitv3902@gmail.com" className="inline-block bg-[#2980b9] text-white px-6 py-4 rounded-2xl font-black uppercase text-sm shadow-lg shadow-blue-900/30 active:scale-95 transition-transform relative z-10">
    ...
    if (paginaActual === 'neumaticos') return (
      <div className="space-y-6 animate-fadeIn pb-10">
        <div className="bg-[#101c33] p-6 rounded-[2.5rem] border border-gray-800 shadow-2xl relative overflow-hidden">
          ...
          <button onClick={generarInformeNeumaticos} className="w-full mt-6 bg-[#2980b9] py-5 rounded-2xl font-black text-sm uppercase shadow-xl shadow-blue-900/30 text-white active:scale-95 transition-transform">
      ...
      {usuarioActual && (
        <>
          ...
          <nav className="fixed bottom-0 left-0 right-0 bg-[#101c33]/95 backdrop-blur-md border-t border-gray-800 p-4 flex justify-around items-center z-50 pb-safe shadow-[0_-10px_30px_rgba(0,0,0,0.5)]">
            {[{id:'buscar', icon:'🔍', label:'BUSCAR'}, esMaestro ? {id:'añadir', icon:'➕', label:'AÑADIR'} : {id:'contacto', icon:'✉️', label:'CONTACTO'}, {id:'neumaticos', icon:'🛞', label:'NEUMÁTICOS'}, {id:'perfil', icon:'👤', label:'PERFIL'}].map(item => (
              ...
              <div className="bg-[#101c33] rounded-[2.5rem] p-8 w-full max-w-md border border-gray-800 flex flex-col max-h-[90vh] shadow-[0_35px_60px_-15px_rgba(0,0,0,0.8)]">
                ...
                {!editandoVehiculo && paginaActual !== 'añadir' && vehiculoModal && (
                  <div className="flex justify-center gap-2 mb-6 bg-[#060c17] p-2 rounded-2xl border border-gray-800 overflow-x-auto custom-scrollbar-thin shadow-inner">
                    ...
                    {vehiculoModal.protocoloHibridoPDF && (
                      <button onClick={() => window.open(vehiculoModal.protocoloHibridoPDF, '_blank')}
                        className="px-3 py-2 rounded-xl text-[9px] font-black uppercase transition-all flex-1 text-center whitespace-nowrap active:scale-95 bg-gray-300 text-black border-2 border-gray-400">
                        Protocolo Híbrido
                      </button>
                    )}
                  </div>
